...
def persian_pdf(scan_result_datetime, device, capacity, serial, total_file,
    ...
    font_directory = os.path.join('websocket_handlers', 'Vazir.ttf')

    # pdfmetrics.registerFont(
        # TTFont('Vaziri', font_directory))
    pdfmetrics.registerFont(
...
def upload_pdf(directory, date, password):
    doc = SimpleDocTemplate(
        r'C:/upload_print.pdf',
        pagesize=(3.16*inch, 8.27*inch),
        rightMargin=10,
        leftMargin=10,
        topMargin=0,
...
#        ]))
    ...
    Story.append(paragraphContext(get_display(
        arabic_reshaper.reshape(u'رمز: ' + password)),
        'titleParph')
    )
